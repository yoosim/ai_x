...
def agreement_edit(request, pk):
    ...
    # 배열/마커 복원
    def _arr(key):
        try: return json.loads(F.get(key) or "[]")
...
def agreement_update(request, pk):
    ...
    ag.incident_dt = F["accident_date"]
    ag.location    = F["location"]
    ag.a_name      = F["a_name"]
    ag.b_name      = F["b_name"]
    ag.damages_raw = json.dumps(F, ensure_ascii=False)
    ag.save(update_fields=["incident_dt","location","a_name","b_name","damages_raw","updated_at"])
...
def agreement_preview_saved(request, pk):
    ...
    return render(
        ...
        "accident_project/print.html",
        {
            ...
            "pk": rec.pk,
            "mask_rrn": mask,
            "in_preview": True,   # ← 미리보기 모드(다운로드/인쇄 숨김)
            "part_anchors": PART_ANCHORS,
            "label_colors": LABEL_COLORS,
        },
    )
