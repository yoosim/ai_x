...
# test2 버전에서 데이터 확보 후 100 -> 900장으로 모델링 진행 
...
def save_training_graph(train_acc_list, val_acc_list, train_loss_list, val_loss_list, lr_list, save_path="0712_test3_training_result.png"):
    ...
    plt.subplot(1, 3, 3)
    # 실제 학습률 변화 시각화
    plt.plot(epochs[:len(lr_list)], lr_list, label='Learning Rate', color='green')
    plt.xlabel('Epoch')
    plt.ylabel('Learning Rate')
    plt.title('Learning Rate Schedule')
    plt.legend()
    plt.yscale('log')  # 로그 스케일로 보기 (학습률이 작아서)
...
if __name__ == "__main__":
    # 데이터 경로
    data_dir = r"C:\Users\baby3\ai_x\source\Project\0711~0713 성미정리\3.ml_data\1. 모델 구성\split_data"    
    ...
    best_val_acc = 0
    early_stop_count = 0
    early_stop_patience = 15  #10번 정체면 저장
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            torch.save(model.state_dict(), "efficientNetB0_0712_test3_best_burger_model.pth")
            print("모델 저장됨 (최고 성능)")
            early_stop_count = 0
        else:
            early_stop_count += 1
    ...
    # 그래프 저장
    save_training_graph(train_acc_list, val_acc_list, train_loss_list, val_loss_list, lr_list)
